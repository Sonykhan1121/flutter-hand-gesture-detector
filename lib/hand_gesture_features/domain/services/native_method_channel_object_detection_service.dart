...
/// Object detector implemented by the app's Android MethodChannel plugin.
final class NativeMethodChannelObjectDetectionService
    extends SingleFlightObjectDetectionService {
  NativeMethodChannelObjectDetectionService._(this._nativeDetector);

  final NativeObjectDetection _nativeDetector;
  var _nextFrameId = 0;
  ...
  static Future<NativeMethodChannelObjectDetectionService> start({
    NativeObjectDetection nativeDetector = const NativeObjectDetection(),
    bool? isAndroid,
  }) async {
    if (!(isAndroid ?? Platform.isAndroid)) {
      throw UnsupportedError(
        'The native MethodChannel object detector currently supports '
        'Android only.',
      );
    }

    await nativeDetector.initialize(
      modelAsset: HandGestureThresholds.nativeMethodChannelModelAsset,
      confidenceThreshold:
          HandGestureThresholds.nativeMethodChannelConfidenceThreshold,
      iouThreshold: HandGestureThresholds.nativeMethodChannelIouThreshold,
      maxResults: HandGestureThresholds.objectDetectionMaxResults,
      expectedClassCount:
          HandGestureThresholds.nativeMethodChannelExpectedClassCount,
      useGpu: HandGestureThresholds.nativeMethodChannelUseGpu,
    );
    final capabilities = await nativeDetector.getCapabilities();
    if (kDebugMode) {
      debugPrint(
        'Native MethodChannel object detector initialized: $capabilities',
      );
    }
    return NativeMethodChannelObjectDetectionService._(nativeDetector);
  }
  ...
  Future<List<AppObjectDetection>> performDetection(
    ...
    if (responseFrameId != frameId ||
      ...
      if (kDebugMode) {
        debugPrint(
          'Native MethodChannel detector ignored mismatched frame response: '
    ...
    if (kDebugMode && (frameId == 1 || frameId % 10 == 0)) {
      ...
      final accelerator = response['accelerator']?.toString() ?? 'unknown';
      debugPrint(
        'Native MethodChannel inference: '
        '${inferenceMs?.toStringAsFixed(1) ?? 'unknown'}ms, '
        'accelerator=$accelerator, detections=${detections.length}.',
      );
    }
    return detections;
  }
  ...
  /// Defensively validates the native response before it enters follow logic.
  @visibleForTesting
    ...
    for (final rawDetection in rawDetections) {
      ...
      results.add(
        AppObjectDetection(
          ...
          source: AppObjectDetectionSource.nativeMethodChannel,
        ),
      );
    }
