...
class HandLandmarks {
  ...
  /// Confidence/probability emitted by the handedness model.
...
class Hand {
  ...
  /// The 21 metric world-space landmarks emitted by the landmark model.
  final List<HandLandmark> worldLandmarks;

  /// Presence/landmark confidence emitted by the landmark model.
  final double? landmarkConfidence;

  /// Probability emitted by the handedness model.
  final double? handednessScore;
  ...
  Map<String, dynamic> toMap() => {
    ...
    'worldLandmarks': worldLandmarks.map((l) => l.toMap()).toList(),
  ...
  static Hand fromMap(Map<String, dynamic> map) => Hand(
    ...
    landmarks: (map['landmarks'] as List<dynamic>)
        ...
        .toList(),
    worldLandmarks:
        (map['worldLandmarks'] as List<dynamic>?)
            ?.map((l) => HandLandmark.fromMap(l as Map<String, dynamic>))
            .toList() ??
        const [],
    landmarkConfidence: (map['landmarkConfidence'] as num?)?.toDouble(),
    handednessScore: (map['handednessScore'] as num?)?.toDouble(),
